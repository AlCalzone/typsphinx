...
class TestSameDocumentReferences:
    ...
    def test_ref_to_section_target_generates_link(self, build_typst):
        ...
        # The section heading must carry the matching label (attached in a
        # markup block, since labels can only be attached in markup mode)
        assert '[#heading(level: 2, text("Target Section")) <target-section>]' in output
    ...
    def test_unreferenced_sections_get_no_label(self, build_typst):
        """Sections that are not referenced must not grow labels (no churn)."""
        ...
        assert 'heading(level: 2, text("Plain Section"))\n' in output
        assert "<plain-section>" not in output
