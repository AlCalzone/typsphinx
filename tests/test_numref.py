...
class TestNumrefNativeReferences:
    ...
    def test_figure_numref_uses_native_ref(self, numref_output):
        """:numref:`fig-example` becomes ref(label("fig-example"))."""
        assert 'ref(label("fig-example"))' in numref_output

    def test_table_numref_uses_native_ref(self, numref_output):
        """:numref:`tbl-example` becomes ref(label("tbl-example"))."""
        assert 'ref(label("tbl-example"))' in numref_output
    ...
    def test_figure_label_is_attached(self, numref_output):
        """The figure must carry the label the ref() points to."""
        assert "<fig-example>" in numref_output

    def test_table_label_is_attached(self, numref_output):
        """The table must carry the label the ref() points to."""
        assert "<tbl-example>" in numref_output
